...
<body>
	...
	<div class="container">
		...
		<form class="clearfix searchform">
		 ...
		 <?php
			$favMovies = $mysql_rpc->call("--GETMOVIE--SELECT movie_id FROM Fav_and_Recommended WHERE email='" . $_SESSION['user'] . "'--GETMOVIE--");
			error_log("[{$date}] [{$file}] [{$level}] Getting Favorite movies for User {$_SESSION['user']}.".PHP_EOL, 3, $LOGFILE);
			if($favMovies=='nomovies'){
				echo '<h4>You have no favorite movies yet.</h4>';
				echo '<br><br>You are being redirected to the home page...';
				header("Refresh: 3; URL=home.php");
				exit;
			}
			$favMoviesArray = explode(",",$favMovies);
			foreach($favMoviesArray as $movieID){
				$send = "";
				...
				array_push($moviePosters, $poster);
